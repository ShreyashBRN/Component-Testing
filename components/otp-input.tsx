 ...
 *                           labeled underneath (e.g. "1. Classic").
...
// Human-readable label for each variant — first word capitalized.
function variantLabel(v: OtpVariant): string {
  const labels: Record<OtpVariant, string> = {
    classic: "Classic",
    underline: "Underline",
    pill: "Capsule",
    filled: "Filled",
...
// Muted caption color for the variant labels beneath each demo — legible on
// both the light and dark showcase backgrounds without competing with the
// input itself.
function labelColor(theme: ThemeMode) {
  return theme === "dark" ? "rgba(255,255,255,0.55)" : "rgba(36,27,61,0.55)";
}
...
export function OtpInput({
  ...
  return (
    <div
  className={cn(
    "grid w-full grid-cols-1 justify-center place-items-center gap-y-[56px] gap-x-6 md:grid-cols-[repeat(2,max-content)] md:gap-y-[152px] md:gap-x-[135px]",
    className
  )}
>
  {variants.map((v, i) => (
    <div key={v} className="flex flex-col items-center justify-center gap-3">
  ...
  return (
    ...
    >
      ...
      <button
        ...
        className={cn(
  "grid w-full grid-cols-1 place-items-center gap-y-[56px] gap-x-6 md:grid-cols-2 md:gap-y-[112px] md:gap-x-0",
)}
      >
        {isDark ? <Sun size={16} /> : <Moon size={16} />}
      </button>

      <div className="relative z-10 w-full max-w-[620px] p-4 md:p-6">

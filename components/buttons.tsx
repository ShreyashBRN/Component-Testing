...
export default function ButtonsShowcase({ variants = DEFAULT_VARIANTS }: ButtonsShowcaseProps) {
  return (
    <div className="grid w-full grid-cols-1 place-items-center gap-y-14 gap-x-[90px] sm:grid-cols-2 sm:gap-y-[100px] lg:gap-y-[60px]">
      {variants.map(({ variant, name }) => (
        <div key={variant} className="flex flex-col items-center gap-4">
          <AnimatedButton variant={variant} />
          <span className="text-sm lg:text-lg text-neutral-500 dark:text-neutral-400">{name}</span>
        </div>
      ))}
    </div>
  );
